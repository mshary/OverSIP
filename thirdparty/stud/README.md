...
`stud` requires:
    ...
    libev >= 4
    openssl >= 1.0.0 (tested with 1.0.x, 1.1.x, and 3.0.x)

Stud currently works on Linux, OpenBSD, FreeBSD, and MacOSX.
It has been tested the most heavily on Linux/x86_64.

Note: Stud has been updated to support OpenSSL 3.0 with all deprecated APIs
properly handled. When using OpenSSL 3.0 or later, ENGINE support is not
available and relies on OpenSSL providers instead.

Modern Security Features
------------------------

`stud` now includes modern security features:

 * **OpenSSL 3.0 Support**: Full compatibility with OpenSSL 3.0.x with proper handling
   of deprecated APIs through version-aware conditional compilation. Supports both
   legacy OpenSSL 1.0.x/1.1.x and modern 3.0.x versions.
 * **TLS Version Control**: Enforce minimum TLS versions (1.1, 1.2, or 1.3) to meet
   security requirements and disable vulnerable protocol versions.
 * **Modern Cryptography**: Automatic support for contemporary cipher suites and
   security standards as provided by the OpenSSL version in use.
...
Detail about the entire set of options can be found by invoking `stud -h`:
    ...
    ENCRYPTION METHODS:
          ...
          --tls11                 Enforce minimum TLS 1.1
          --tls12                 Enforce minimum TLS 1.2
          --tls13                 Enforce minimum TLS 1.3
...
By default, `stud` allows negotiation of any supported TLS version. For enhanced security,
you can enforce a minimum TLS version using the following options:

    --tls11    Enforce minimum TLS 1.1
    --tls12    Enforce minimum TLS 1.2
    --tls13    Enforce minimum TLS 1.3

For example, to require TLS 1.2 or higher:

    stud --tls12 /path/to/certificate.pem

Or to require the latest TLS 1.3:

    stud --tls13 /path/to/certificate.pem

These options are useful for enforcing modern security standards and disabling 
deprecated or vulnerable protocol versions.
