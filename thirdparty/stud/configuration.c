...
#define CFG_TLS_VERSION_MIN "tls-min-version"
#define CFG_TLS_VERSION_MAX "tls-max-version"
...
#define CFG_PARAM_TLS11 11016
#define CFG_PARAM_TLS12 11017
#define CFG_PARAM_TLS13 11018
...
stud_config * config_new (void) {
  ...
  r->ETYPE              = ENC_TLS;
  r->PMODE              = SSL_SERVER;
  r->TLS_VERSION_MIN    = TLS_VERSION_AUTO;
  r->TLS_VERSION_MAX    = TLS_VERSION_AUTO;
  r->WRITE_IP_OCTET     = 0;
...
void config_print_usage_fd (char *prog, stud_config *cfg, FILE *out) {
  ...
  fprintf(out, "      --tls11                 Enforce minimum TLS 1.1\n");
  fprintf(out, "      --tls12                 Enforce minimum TLS 1.2\n");
  fprintf(out, "      --tls13                 Enforce minimum TLS 1.3\n");
...
void config_print_default (FILE *fd, stud_config *cfg) {
  ...
  fprintf(fd, "# SSL/TLS protocol version.\n");
  fprintf(fd, "#\n");
  fprintf(fd, "# Supported versions:\n");
  fprintf(fd, "# - tls = on (default, supports TLSv1.0 and later)\n");
  fprintf(fd, "# - ssl = off (disables SSLv3)\n");
  fprintf(fd, "# - tls11 = on (enforce minimum TLSv1.1)\n");
  fprintf(fd, "# - tls12 = on (enforce minimum TLSv1.2)\n");
  fprintf(fd, "# - tls13 = on (enforce minimum TLSv1.3)\n");
  fprintf(fd, "#\n");
  fprintf(fd, "# tls = on\n");
  fprintf(fd, "# ssl = off\n");
  fprintf(fd, "\n");
...
void config_parse_cli(int argc, char **argv, stud_config *cfg) {
  ...
  struct option long_options[] = {
    ...
    { "tls", 0, &tls, 1},
    { "ssl", 0, &ssl, 1},
    { "tls11", 0, NULL, CFG_PARAM_TLS11 },
    { "tls12", 0, NULL, CFG_PARAM_TLS12 },
    { "tls13", 0, NULL, CFG_PARAM_TLS13 },    
    { "client", 0, &client, 1},    
  ...
  while (1) {
    ...
    switch (c) {
      ...
      case CFG_PARAM_SYSLOG_FACILITY:
        ...
        break;
      case CFG_PARAM_TLS11:
        cfg->TLS_VERSION_MIN = TLS_VERSION_11;
        cfg->TLS_VERSION_MAX = TLS_VERSION_AUTO;
        break;
      case CFG_PARAM_TLS12:
        cfg->TLS_VERSION_MIN = TLS_VERSION_12;
        cfg->TLS_VERSION_MAX = TLS_VERSION_AUTO;
        break;
      case CFG_PARAM_TLS13:
        cfg->TLS_VERSION_MIN = TLS_VERSION_13;
        cfg->TLS_VERSION_MAX = TLS_VERSION_AUTO;
        break;
      case 'c':
