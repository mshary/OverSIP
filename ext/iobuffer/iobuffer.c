...
/* Forward declaration of buffer structure */
struct buffer;

/* Forward declarations for TypedData structure */
static void IO_Buffer_mark(struct buffer *);
static void IO_Buffer_free(struct buffer *);

/* TypedData structure for IO::Buffer */
static const rb_data_type_t buffer_data_type = {
    "IO::Buffer",
    {
        (void (*)(void*))IO_Buffer_mark,
        (void (*)(void*))IO_Buffer_free,
        NULL, /* memsize */
        NULL, /* compact */
    },
    NULL, /* parent */
...
#else
/* Use rb_io_descriptor for modern Ruby to avoid deprecation warning */
/* Note: rb_io_descriptor expects a VALUE (Ruby IO object), not rb_io_t* */
/* We'll keep using fptr->fd for compatibility, but suppress warning */
...
{
    return TypedData_Wrap_Struct(klass, &buffer_data_type, buffer_new());
}
...
IO_Buffer_initialize(int argc, VALUE * argv, VALUE self)
{
    ...
    if (rb_scan_args(argc, argv, "01", &node_size_obj) == 1) {
        TypedData_Get_Struct(self, struct buffer, &buffer_data_type, buf);
...
IO_Buffer_read_from(VALUE self, VALUE io)
{
    ...
    /* Use rb_io_descriptor to avoid deprecation warning */
...
buffer_read_frame(struct buffer * buf, VALUE str, char frame_mark)
{
    ...
    while (buf->size > 0) {
        ...
        char           *loc, *s = (char *)(head->data + head->start), *e = (char *)(head->data + head->end);
        nbytes = e - s;
